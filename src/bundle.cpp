...
static Result<void> create_squashfs(const std::string& content_dir, const std::string& output_path,
                                    const std::string& extra_args) {
    std::vector<std::string> cmd = {"mksquashfs", content_dir, output_path, "-all-root",
                                    "-noappend"};
    if (!extra_args.empty()) {
...
Result<void> bundle_create(const BundleCreateParams& params) {
    ...
    for (auto& img : manifest.images) {
        ...
        img.size = cs.size;
        LOG_INFO("Image %s: sha256=%s size=%lu", img.filename.c_str(), cs.digest.c_str(), cs.size);
    }
    ...
    auto sqres = create_squashfs(params.content_dir, squashfs_path, params.mksquashfs_args);
    if (!sqres)
        return sqres;
    ...
    if (params.format == BundleFormat::Plain) {
        ...
        LOG_INFO("Verity root_hash=%s salt=%s hash_size=%lu", verity.root_hash.c_str(),
                 verity.salt.c_str(), verity.hash_size);
...
static Result<Bytes> extract_bundle_signature(const std::string& path, uint64_t& payload_size) {
    ...
    if (!f)
        return Result<Bytes>::err("Cannot open bundle");
...
Result<Bundle> bundle_open(const std::string& path, const SigningParams& verify_params) {
    ...
    try {
        ...
        try {
            ...
            if (!ctx.config().encryption_key.empty()) {
                auto decrypted =
                    cms_decrypt(sig, ctx.config().encryption_key, ctx.config().encryption_cert);
    ...
    LOG_INFO("Bundle opened: format=%s compatible=%s version=%s", to_string(bundle.format),
             bundle.manifest.compatible.c_str(), bundle.manifest.version.c_str());
...
Result<void> bundle_mount(Bundle& bundle) {
    ...
    try {
        if (bundle.format == BundleFormat::Plain) {
            ...
            bundle.dm_verity =
                dm_verity_setup(bundle.loop.path, data_size, bundle.manifest.verity_hash,
                                bundle.manifest.verity_salt, data_size);

            std::string mp = create_mount_point(mount_prefix, "bundle");
            auto res = mount(bundle.dm_verity.dm_device, mp, "squashfs", MS_RDONLY);
            if (!res)
                return Result<void>::err("Mount verity bundle failed: " + res.error());
            bundle.mount_point = mp;

        } else if (bundle.format == BundleFormat::Crypt) {
            const uint64_t enc_size = payload_size - bundle.manifest.bundle_verity_size;

            bundle.loop = loop_setup(bundle.path, 0, payload_size);

            bundle.dm_crypt = dm_crypt_setup(bundle.loop.path, enc_size, bundle.manifest.crypt_key);

            bundle.dm_verity =
                dm_verity_setup(bundle.dm_crypt.dm_device, enc_size, bundle.manifest.verity_hash,
                                bundle.manifest.verity_salt, enc_size);

            std::string mp = create_mount_point(mount_prefix, "bundle");
            auto res = mount(bundle.dm_verity.dm_device, mp, "squashfs", MS_RDONLY);
            if (!res)
                return Result<void>::err("Mount crypt bundle failed: " + res.error());
            bundle.mount_point = mp;
        }
...
Result<Bundle> bundle_info(const std::string& path, const SigningParams& params, bool no_verify) {
    if (no_verify) {
