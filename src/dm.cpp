...
void set_dm_header(dm_ioctl& hdr, size_t total_size, uint32_t flags, const std::string& name) {
...
int open_dm_control() {
    ...
    if (fd < 0) {
        throw AegisError("Failed to open /dev/mapper/control: " +
                         std::string(std::strerror(errno)));
    }
    return fd;
}

std::string build_verity_params(const std::string& data_device, uint64_t data_size,
                                const std::string& root_hash, const std::string& salt,
    ...
    return "1 " + data_device + " " + data_device + " 4096 4096 " + std::to_string(data_blocks) +
           " " + std::to_string(hash_start_block) + " sha256 " + root_hash + " " + salt;
}

std::string build_crypt_params(const std::string& data_device, const std::string& hex_key,
...
void quick_check_device(const std::string& path) {
    ...
    if (fd < 0) {
        throw AegisError("Failed to open mapped device " + path + ": " +
                         std::string(std::strerror(errno)));
    }
    ...
    if (rd != 1) {
        throw AegisError("Check read from mapped device failed: " +
                         std::string(std::strerror(saved_errno)));
    }
}

DmTarget create_dm_target(const std::string& dm_name, const std::string& target_type,
                          uint64_t data_size, const std::string& params) {
    if (data_size == 0) {
    ...
    try {
        ...
        std::snprintf(setup.target.target_type, sizeof(setup.target.target_type), "%s",
                      target_type.c_str());
...
LoopDevice loop_setup(const std::string& file_path, uint64_t offset, uint64_t size) {
    ...
    try {
        ...
        if (backing_fd < 0) {
            throw AegisError("Failed to open bundle file " + file_path + ": " +
                             std::string(std::strerror(errno)));
        }

        control_fd = ::open("/dev/loop-control", O_RDWR | O_CLOEXEC);
        if (control_fd < 0) {
            throw AegisError("Failed to open /dev/loop-control: " +
                             std::string(std::strerror(errno)));
        }
        ...
        for (int tries = 0; tries < 10; ++tries) {
            ...
            if (loop_idx < 0) {
                throw AegisError("Failed to get free loop device: " +
                                 std::string(std::strerror(errno)));
            }
            ...
            if (loop_fd < 0) {
                ...
                throw AegisError("Failed to open " + loop_path + ": " +
                                 std::string(std::strerror(errno)));
            }
            ...
            if (::ioctl(loop_fd, LOOP_SET_FD, backing_fd) == 0) {
                ...
                if (::ioctl(loop_fd, LOOP_SET_STATUS64, &info) != 0) {
                    ...
                    ::close(loop_fd);
                    throw AegisError("Failed to configure loop device: " +
                                     std::string(std::strerror(saved_errno)));
                }
                ...
                LOG_INFO("Loop device setup: %s -> %s (offset=%lu size=%lu)", file_path.c_str(),
                         loop.path.c_str(), offset, size);
                return loop;
            }
            ...
            throw AegisError("Failed to bind file to loop device: " +
                             std::string(std::strerror(saved_errno)));
        }
...
Result<void> loop_teardown(const LoopDevice& loop) {
    ...
    if (fd < 0) {
        ...
        if (fd < 0) {
            return Result<void>::err("Failed to open loop device " + loop.path + ": " +
                                     std::string(std::strerror(errno)));
        }
    }
    ...
    if (::ioctl(fd, LOOP_CLR_FD, 0) != 0) {
        ...
        return Result<void>::err("Failed to detach loop device " + loop.path + ": " +
                                 std::string(std::strerror(saved_errno)));
    }
...
DmTarget dm_verity_setup(const std::string& data_device, uint64_t data_size,
                         const std::string& root_hash, const std::string& salt,
                         uint64_t hash_offset) {
...
DmTarget dm_crypt_setup(const std::string& data_device, uint64_t data_size,
                        const std::string& hex_key, const std::string& cipher) {
...
Result<void> dm_remove(const std::string& dm_name) {
    ...
    if (dmfd < 0) {
        return Result<void>::err("Failed to open /dev/mapper/control: " +
                                 std::string(std::strerror(errno)));
    }
    ...
    if (::ioctl(dmfd, DM_DEV_REMOVE, &setup) != 0) {
        int saved_errno = errno;
        ::close(dmfd);
        return Result<void>::err("Failed to remove dm device " + dm_name + ": " +
                                 std::string(std::strerror(saved_errno)));
    }
