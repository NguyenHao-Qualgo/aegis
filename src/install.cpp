...
Result<std::vector<InstallPlan>> make_install_plans(const Manifest& manifest,
    ...
    for (auto& image : manifest.images) {
        ...
        if (it == target_group.end()) {
            return Result<std::vector<InstallPlan>>::err("No target slot for class '" +
                                                         image.slotclass + "' of image '" +
                                                         image.filename + "'");
        }

        if (it->second->readonly) {
            return Result<std::vector<InstallPlan>>::err("Target slot for class '" +
                                                         image.slotclass + "' is readonly");
        }
        ...
        LOG_INFO("Plan: %s -> %s (%s)", image.filename.c_str(), it->second->name.c_str(),
