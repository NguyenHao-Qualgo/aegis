...
Bytes read_file_bytes(const std::string& path) {
    ...
    if (!f)
        throw SignatureError("Cannot open: " + path);
    fseek(f, 0, SEEK_END);
    long sz = ftell(f);
    fseek(f, 0, SEEK_SET);
    Bytes data(sz);
...
void write_file_bytes(const std::string& path, const Bytes& data) {
    ...
    if (!f)
        throw SignatureError("Cannot write: " + path);
...
struct CmsDeleter {
    void operator()(CMS_ContentInfo* c) const {
...
static UniqueX509 load_cert(const std::string& path) {
    ...
    if (!f)
        throw SignatureError("Cannot open cert: " + path);
    X509* cert = PEM_read_X509(f, nullptr, nullptr, nullptr);
    fclose(f);
    if (!cert)
        throw SignatureError("Failed to read cert: " + path);
...
static UniquePkey load_key(const std::string& path) {
    FILE* f = fopen(path.c_str(), "r");
    if (!f)
        throw SignatureError("Cannot open key: " + path);
    EVP_PKEY* key = PEM_read_PrivateKey(f, nullptr, nullptr, nullptr);
    fclose(f);
    if (!key)
        throw SignatureError("Failed to read key: " + path);
...
static UniqueStore build_trust_store(const SigningParams& params) {
    ...
    if (!store)
        throw SignatureError("X509_STORE_new failed");
...
Bytes cms_sign(const Bytes& data, const SigningParams& params) {
    ...
    if (!in)
        throw SignatureError("BIO_new_mem_buf failed");

    int flags = CMS_BINARY;
    CMS_ContentInfo* cms = CMS_sign(cert.get(), key.get(), nullptr, in.get(), flags);
    if (!cms)
        throw SignatureError("CMS_sign failed: " + openssl_error_string());
...
Bytes cms_sign_file(const std::string& path, uint64_t size, const SigningParams& params) {
    ...
    if (!in)
        throw SignatureError("Cannot open for signing: " + path);

    int flags = CMS_BINARY | CMS_DETACHED;
    CMS_ContentInfo* cms = CMS_sign(cert.get(), key.get(), nullptr, in.get(), flags);
    if (!cms)
        throw SignatureError("CMS_sign_file failed: " + openssl_error_string());
    UniqueCms cms_guard(cms);
...
Bytes cms_verify(const Bytes& cms_data, const SigningParams& params) {
    ...
    if (!cms)
        throw SignatureError("d2i_CMS_bio failed: " + openssl_error_string());
    UniqueCms cms_guard(cms);
...
Result<void> cms_verify_content(const Bytes& cms_data, const Bytes& content,
    ...
    CMS_ContentInfo* cms = d2i_CMS_bio(cms_bio.get(), nullptr);
    if (!cms)
        return Result<void>::err("d2i_CMS_bio failed");
    UniqueCms cms_guard(cms);
...
Bytes cms_encrypt(const Bytes& data, const std::vector<std::string>& recipient_certs) {
    ...
    UniqueBio in(BIO_new_mem_buf(data.data(), static_cast<int>(data.size())));
    CMS_ContentInfo* cms = CMS_encrypt(certs, in.get(), EVP_aes_256_cbc(), CMS_BINARY | CMS_STREAM);
    sk_X509_free(certs);
    if (!cms)
        throw SignatureError("CMS_encrypt failed: " + openssl_error_string());
    UniqueCms cms_guard(cms);
...
Bytes cms_decrypt(const Bytes& data, const std::string& key_path, const std::string& cert_path) {
    ...
    if (!cms)
        throw SignatureError("d2i_CMS_bio decrypt failed");
    UniqueCms cms_guard(cms);
