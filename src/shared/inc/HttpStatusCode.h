...
enum class HttpStatusCode {
    ...
    SwitchingProtocols = 101,  //!< Indicates that the server understands and is willing to comply
                               ...
                               //!< connection.
    Processing = 102,          //!< Is an interim response used to inform the client that the server
    ...
    NonAuthoritativeInformation = 203,  //!< Indicates that the request was successful but the enclosed
    ...
    AlreadyReported = 208,              //!< Used inside a DAV:propstat response element to avoid enumerating
    ...
    MovedPermanently = 301,   //!< Indicates that the target resource has been assigned a new
    ...
    MethodNotAllowed = 405,             //!< Indicates that the method specified in the request-line is known
    ...
    NotAcceptable = 406,                //!< Indicates that the target resource does not have a current
                                        ...
                                        //!< representation.
    ProxyAuthenticationRequired = 407,  //!< Is similar to 401 (Unauthorized), but indicates that the client
    ...
    UnsupportedMediaType = 415,         //!< Indicates that the origin server is refusing to service the
                                        //!< request because the payload is in a format not supported by the
                                        //!< target resource for this method.
    RangeNotSatisfiable = 416,          //!< Indicates that none of the ranges in the request's Range header
    ...
    FailedDependency = 424,             //!< Means that the method could not be performed on the resource
    ...
    RequestHeaderFieldsTooLarge = 431,  //!< Indicates that the server is unwilling to process the request
                                        //!< because its header fields are too large.
    UnavailableForLegalReasons = 451,   //!< This status code indicates that the server is denying access to
    ...
    HTTPVersionNotSupported = 505,        //!< Indicates that the server does not support, or refuses to
                                          //!< support, the protocol version that was used in the request
                                          //!< message.
    VariantAlsoNegotiates = 506,          //!< Indicates that the server has an internal configuration error:
    ...
    InsufficientStorage = 507,            //!< Means the method could not be performed on the resource because
                                          //!< the server is unable to store the representation needed to
                                          //!< successfully complete the request.
    LoopDetected = 508,                   //!< Indicates that the server terminated an operation because it
