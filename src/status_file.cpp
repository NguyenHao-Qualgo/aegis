...
static std::string serialize_slot_status(const std::string& slot_name,
                                          const SlotStatus& s) {
...
static void parse_status_section(const std::map<std::string, std::string>& sec,
                                  SlotStatus& s) {
...
Result<void> load_all_slot_status(std::map<std::string, Slot>& slots,
    ...
    if (!path_exists(status_file_path)) return Result<void>::ok();

    std::ifstream f(status_file_path);
    if (!f) return Result<void>::err("Cannot open: " + status_file_path);
    ...
    for (auto& [sec_name, sec] : sections) {
        if (sec_name.substr(0, prefix.size()) != prefix) continue;
...
Result<void> save_all_slot_status(const std::map<std::string, Slot>& slots,
    ...
    if (!f) return Result<void>::err("Cannot write: " + status_file_path);
