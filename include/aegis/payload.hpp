...
void stream_plain_payload(StreamReader &reader,
    ...
    log_stream("streaming payload '" + entry.name + "' directly from SWU stream, size=" +
    ...
    log_stream("finished streaming payload '" + entry.name + "', streamed=" +
               std::to_string(entry.size) + " bytes");
}
...
void stream_encrypted_payload(StreamReader &reader,
    ...
    log_stream("streaming encrypted payload '" + entry.name +
    ...
    log_stream("finished decrypted streaming for payload '" + entry.name + "'");
}
