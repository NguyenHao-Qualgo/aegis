...
public:
    explicit PayloadStreamer(const InstallContext& ctx) : ctx_(ctx) {}

    template <typename Sink>
    void stream_plain(StreamReader& reader,
        ...
        if (checksum != entry.checksum) {
            fail_runtime("cpio checksum mismatch for " + entry.name);
        }
    ...
    template <typename Sink>
    void stream_encrypted(StreamReader& reader,
                          const CpioEntry& entry,
        ...
        const std::vector<unsigned char> iv =
            hex_to_bytes(ivt_override.empty() ? aes.iv_hex : ivt_override);
        const EVP_CIPHER* cipher = evp_cipher_for_key_length(key.size());
        if (cipher == nullptr) {
            fail_runtime("unsupported AES key length for encrypted payload");
        }
        ...
        while (remaining > 0) {
            ...
            const std::size_t chunk =
                static_cast<std::size_t>(std::min<std::uint64_t>(remaining, inbuf.size()));
            reader.read_exact(inbuf.data(), chunk);
            for (std::size_t i = 0; i < chunk; ++i) {
                checksum += static_cast<unsigned char>(inbuf[i]);
            }
            ...
            if (EVP_DecryptUpdate(ctx.get(),
                ...
                const std::string errors = collect_openssl_errors();
                fail_runtime("EVP_DecryptUpdate failed" +
                             (errors.empty() ? std::string() : "\n" + errors));
            }

            if (outlen > 0) {
                sink(reinterpret_cast<const char*>(outbuf.data()), static_cast<std::size_t>(outlen));
            }
            remaining -= chunk;
        }
        ...
        if (EVP_DecryptFinal_ex(ctx.get(), outbuf.data(), &final_len) != 1) {
            const std::string errors = collect_openssl_errors();
            fail_runtime("EVP_DecryptFinal_ex failed" +
                         (errors.empty() ? std::string() : "\n" + errors));
        }
        if (final_len > 0) {
            sink(reinterpret_cast<const char*>(outbuf.data()), static_cast<std::size_t>(final_len));
        }
