...
The `aes-key` file is expected to contain two bare hex values:
...
The example above shows the 32-byte key format generated by this repository's helper scripts. At runtime, Aegis selects the AES-CBC variant from the key length in this file.

If a manifest entry contains `ivt`, that per-entry value overrides the default IV from `aes-key` for that payload.
...
Install requests are asynchronous. The `Install` DBus method returns quickly, and the actual work runs on the service's background install thread.
